...
class HomiyArizlariNamelari(models.Manager):
    def get_arizasi_tasdiqlangan_homiylar(self):
        return (
            super()
            .get_queryset()
    ...
    def update_homiy_balans(self, data: dict):
        ismi = data["Ismi"]
        summa = data["Balans"]
        ...
        return (
            ...
            .update(
                Balans=decimal.Decimal(summa),
                Sarflangan_summa=decimal.Decimal(sarflangan_summa),
            )
        )
    ...
    def balans_calculate(self, Ismi: str, summa: int):
        ...
        homiy_balans = (
            super().get_queryset().filter(ariza_holati="Tasdiqlandi").get(Ismi=Ismi)
        )
    ...
    def auto_update_balans_calculate(self, Ismi: str, qoldiq: int):
        homiy_balans = (
            super().get_queryset().filter(ariza_holati="Tasdiqlandi").get(Ismi=Ismi)
        )
        balans = (int(homiy_balans.Balans) - int(qoldiq)) + int(
            homiy_balans.Sarflangan_summa
        )
        return balans
