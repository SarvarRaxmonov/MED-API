...
from .serializers import (
    HomiyArizaSerializer,
    HomiyArizasiTahrirlash,
    Homiy_pul_taqsimlash_Serialzier,
)
...
class Homiy_arizasi(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None, *args, **kwargs):
        ...
        serializer = HomiyArizasiTahrirlash(
...
# YANGI FEATURE MAN TOMONIDAN QUSHILGAN MED API GA
...
class Automatic_pul_taqsimlash_view(viewsets.ModelViewSet):
    ...
    def put(self, request):
        instance = HomiyArizasi.objects.filter(
            Ismi=request.data["Ismi"], ariza_holati="Tasdiqlandi"
        ).first()
