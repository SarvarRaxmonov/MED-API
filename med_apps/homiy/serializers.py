...
class HomiyArizaSerializer(serializers.Serializer):
    ...
    def validate_Ismi(self, value):
        all_data = HomiyArizasi.objects.filter(Ismi=value, ariza_holati="Yangi")
        if all_data:
            raise serializers.ValidationError(
                "Uzur siz avval request yuborgansiz sizning so'rovingiz kutish bulimida"
            )
        else:
            return value
...
class Homiy_pul_taqsimlash_Serialzier(serializers.ModelSerializer):
    ...
    def validate_talaba_soni(self, value):
        ...
        if value > jami_talabalar:
            raise ValidationError(
                f"Bizda {value} ta talaba yuq siz noto'gri raqam kiritingiz"
            )
        return value
    ...
    def validate(self, attrs):
        ...
        shuncha_talaba_chiqadimi = (
            Talaba_qushish.objects.auto_update_uchun_talabalar_soni_checking(attrs)
        )
        if foizdan > 100 or foizgacha > 100:
            raise ValidationError("Iltimos foizni to'g'ri kiriting")
        elif shuncha_talaba_chiqadimi == False:
            raise ValidationError(
                "Iltimos buncha kup talaba yuq bizda tulangan kontrakti"
            )
    ...
    def update(self, instance, validated_data):
        ...
        homiy_sarflagan_summa = HomiyArizasi.objects.auto_update_balans_calculate(
            Ismi=validated_data["Ismi"], qoldiq=update_talaba["ortib_qolgan_summa"]
        )
        if validated_data:
            validated_data["Sarflangan_summa"] = homiy_sarflagan_summa
            validated_data["Balans"] = update_talaba["ortib_qolgan_summa"]
