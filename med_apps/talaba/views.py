...
from .serializers import Talaba_Qushish_serializer , Talabaga_homiy_qushish_serializer, Talaba_data_Read_Only_serializer
...
class Talaba_qushish_view(ModelViewSet):
    ...
    @action(detail=False,methods=['post','get'],serializer_class=Talaba_Qushish_serializer)  
    ...
    @action(detail=True,methods=['post','get'],serializer_class=Talabaga_homiy_qushish_serializer)
    def add_homiy(self,request,pk=None):
        if request.method == "GET":
           queryset = get_object_or_404(Talaba_qushish.objects.all(),pk=pk)
           return Response(Talaba_data_Read_Only_serializer(queryset,many=False,context={'request': request}).data)
